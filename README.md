...
- XCache ：
    - XGocache ：单机并发安全的的高性能缓存，用于单机数据缓存，仅支持一些简单数据结构的的缓存操作，基于github.com/pmylund/go-cache包
    - XRedis ： 可分布式的高性能缓存服务连接池，用于分布式数据缓存，支持复杂数据结构的缓存操作，基于 https://github.com/gomodule/redigo 包
