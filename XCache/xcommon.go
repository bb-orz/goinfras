...
type ICommonCache interface {
	...
	AddWithExp(k string, v interface{}, exp time.Duration) error     // 添加一个不存在或已超时的键值，带超时
	GetWithExp(k string) (interface{}, time.Time, bool)              // 获取一个带过期时间的键值
	SetWithExp(k string, v interface{}, exp time.Duration) error     // 更新或添加一个键值，无论是否已存在，带超时
	ReplaceWithExp(k string, v interface{}, exp time.Duration) error // 更新一个已存在且未过期的键值，不满足条件则报错，带超时
